<section class="pricing-plans-section">
  <div class="container-xl d-flex flex-column">
    <div class="section-header row gx-6">
      <div class="col-lg-5 text-lg-end">
        <h2 class="section-title mb-0 text-uppercase" data-aos="fade-right">Investment</h2>
      </div>
      <div class="col-lg-6 section-description d-flex flex-column">
        <p class="mb-0" data-aos="fade-left">Zoom Wedding Studio has the talent and tools to fit your needs and customize any package of your choice.</p>
        <p class="lead mb-0" data-aos="fade-left" data-aos-delay="300"><strong>The price starts at $1,550!</strong></p>
      </div>  
    </div>
    <div class="pricing-tabs-section d-flex flex-column">
      <div class="btn-group d-flex justify-content-center align-self-center" role="group" aria-label="Pricing radio toggle button group" data-aos="fade-up">
        <input 
          type="radio" 
          class="btn-check" 
          name="pricing" 
          id="pricing-photo-video" 
          value="Photo+Video" 
          autocomplete="off" 
          checked=""
        >
        <label 
          class="btn btn-outline-primary" 
          for="pricing-photo-video"
        >
          Photo+Video
        </label>
        <input 
          type="radio" 
          class="btn-check" 
          name="pricing" 
          id="pricing-photo" 
          value="Photo" 
          autocomplete="off"
        >
        <label 
          class="btn btn-outline-primary" 
          for="pricing-photo"
        >
          Photo
        </label>
        <input 
          type="radio" 
          class="btn-check" 
          name="pricing" 
          id="pricing-video" 
          value="Video" 
          autocomplete="off"
        >
        <label 
          class="btn btn-outline-primary" 
          for="pricing-video"
        >
          Video
        </label>
      </div>
      <div class="tab-content">
        <div class="tab-pane fade show active" id="pricing-photo-video-tab" role="tabpanel" aria-labelledby="pricing-photo-video">
          <div class="row justify-content-center">
            <div class="col-lg-3" data-aos="fade-up">
              <div class="pricing-plan-card d-flex flex-column align-items-center text-center h-100">
                <h3 class="plan-title mb-0 d-flex flex-column align-items-center text-uppercase w-100">
                  <svg width="59" height="50" viewBox="0 0 59 50">
                    <image width="59" height="50" xlink:href="data:img/png;base64,"/>
                  </svg>
                  HIGHLIGHT
                </h3>
                <p class="plan-subtitle mb-0 text-center w-100 text-uppercase">4 HOURS COVERAGE</p>
                <ul class="plan-general-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>1-2 PHOTOGRAPHERS</li>
                  <li>1 VIDEOGRAPHER</li>
                </ul>
                <ul class="plan-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>400+ Edited Images</li>
                  <li>10 min. Highlight Video</li>
                  <li>2 Video Cameras</li>
                  <li>1 Wireless Microphone</li>
                  <li>Editing Style options</li>
                  <li>Unlimited Locations</li>
                  <li>Online Photo Gallery</li>
                  <li>Digital Download</li>
                  <li>All print rights</li>
                  <li>Music License</li>
                  <li>Online viewing</li>
                </ul>
                <a href="#" class="btn btn-primary btn-lg d-flex align-items-center justify-content-center mt-auto w-100">
                  Get a quote
                  <svg class="svg-arrow-right" width="15" height="15" viewBox="0 0 24 24" fill="none">
                    <path d="M16.3153 16.6681C15.9247 17.0587 15.9247 17.6918 16.3153 18.0824C16.7058 18.4729 17.339 18.4729 17.7295 18.0824L22.3951 13.4168C23.1761 12.6357 23.1761 11.3694 22.3951 10.5883L17.7266 5.9199C17.3361 5.52938 16.703 5.52938 16.3124 5.91991C15.9219 6.31043 15.9219 6.9436 16.3124 7.33412L19.9785 11.0002L2 11.0002C1.44772 11.0002 1 11.4479 1 12.0002C1 12.5524 1.44772 13.0002 2 13.0002L19.9832 13.0002L16.3153 16.6681Z" fill="#0F0F0F"/>
                  </svg>
                </a>
              </div>
            </div>
            <div class="col-lg-3" data-aos="fade-up" data-aos-delay="300">
              <div class="pricing-plan-card d-flex flex-column align-items-center text-center h-100">
                <h3 class="plan-title mb-0 d-flex flex-column align-items-center text-uppercase w-100">
                  <svg width="59" height="50" viewBox="0 0 59 50">
                    <image width="59" height="50" xlink:href="data:img/png;base64,"/>
                  </svg>
                  GOLD
                </h3>
                <p class="plan-subtitle mb-0 text-center w-100 text-uppercase">6 HOURS COVERAGE</p>
                <ul class="plan-general-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>1-2 PHOTOGRAPHERS</li>
                  <li>1 VIDEOGRAPHER</li>
                </ul>
                <ul class="plan-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>600+ Edited Images</li>
                  <li>3 min. Highlight Video</li>
                  <li>20 min. Full film</li>
                  <li>2 Video Cameras</li>
                  <li>2 Wireless Microphones</li>
                  <li>Aerial Drone Footege</li>
                  <li>Editing Style options</li>
                  <li>Unlimited Locations</li>
                  <li>Online Photo Gallery</li>
                  <li>Digital Download</li>
                  <li>All print rights</li>
                  <li>Music License</li>
                </ul>
                <a href="#" class="btn btn-primary btn-lg d-flex align-items-center justify-content-center mt-auto w-100">
                  Get a quote
                  <svg class="svg-arrow-right" width="15" height="15" viewBox="0 0 24 24" fill="none">
                    <path d="M16.3153 16.6681C15.9247 17.0587 15.9247 17.6918 16.3153 18.0824C16.7058 18.4729 17.339 18.4729 17.7295 18.0824L22.3951 13.4168C23.1761 12.6357 23.1761 11.3694 22.3951 10.5883L17.7266 5.9199C17.3361 5.52938 16.703 5.52938 16.3124 5.91991C15.9219 6.31043 15.9219 6.9436 16.3124 7.33412L19.9785 11.0002L2 11.0002C1.44772 11.0002 1 11.4479 1 12.0002C1 12.5524 1.44772 13.0002 2 13.0002L19.9832 13.0002L16.3153 16.6681Z" fill="#0F0F0F"/>
                  </svg>
                </a>
              </div>
            </div>
            <div class="col-lg-3" data-aos="fade-up" data-aos-delay="600">
              <div class="pricing-plan-card d-flex flex-column align-items-center text-center h-100">
                <h3 class="plan-title mb-0 d-flex flex-column align-items-center text-uppercase w-100">
                  <svg width="59" height="50" viewBox="0 0 59 50">
                    <image width="59" height="50" xlink:href="data:img/png;base64,"/>
                  </svg>
                  PLATINUM
                </h3>
                <p class="plan-subtitle mb-0 text-center w-100 text-uppercase">8 HOURS COVERAGE</p>
                <ul class="plan-general-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>1-2 PHOTOGRAPHERS</li>
                  <li>1 VIDEOGRAPHER</li>
                </ul>
                <ul class="plan-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>800+ Edited Images</li>
                  <li>10-20 Sneak Picks</li>
                  <li>Engagment Session</li>
                  <li>4 min. Highlight Video</li>
                  <li>30 min. Full film</li>
                  <li>2 Video Cameras</li>
                  <li>2 Wireless Microphones</li>
                  <li>Aerial Drone Footege</li>
                  <li>Editing Style options</li>
                  <li>Multiple Locations</li>
                  <li>Online Photo Gallery</li>
                  <li>Digital Download</li>
                  <li>All print rights</li>
                  <li>Music License</li>
                </ul>
                <a href="#" class="btn btn-primary btn-lg d-flex align-items-center justify-content-center mt-auto w-100">
                  Get a quote
                  <svg class="svg-arrow-right" width="15" height="15" viewBox="0 0 24 24" fill="none">
                    <path d="M16.3153 16.6681C15.9247 17.0587 15.9247 17.6918 16.3153 18.0824C16.7058 18.4729 17.339 18.4729 17.7295 18.0824L22.3951 13.4168C23.1761 12.6357 23.1761 11.3694 22.3951 10.5883L17.7266 5.9199C17.3361 5.52938 16.703 5.52938 16.3124 5.91991C15.9219 6.31043 15.9219 6.9436 16.3124 7.33412L19.9785 11.0002L2 11.0002C1.44772 11.0002 1 11.4479 1 12.0002C1 12.5524 1.44772 13.0002 2 13.0002L19.9832 13.0002L16.3153 16.6681Z" fill="#0F0F0F"/>
                  </svg>
                </a>
              </div>
            </div>
            <div class="col-lg-3" data-aos="fade-up" data-aos-delay="900">
              <div class="pricing-plan-card d-flex flex-column align-items-center text-center h-100">
                <h3 class="plan-title mb-0 d-flex flex-column align-items-center text-uppercase w-100">
                  <svg width="59" height="50" viewBox="0 0 59 50">
                    <image width="59" height="50" xlink:href="data:img/png;base64,"/>
                  </svg>
                  BRILLIANT
                </h3>
                <p class="plan-subtitle mb-0 text-center w-100 text-uppercase">10 HOURS COVERAGE</p>
                <ul class="plan-general-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>1-2 PHOTOGRAPHERS</li>
                  <li>1-2 VIDEOGRAPHER</li>
                </ul>
                <ul class="plan-features-list list-unstyled mb-0 d-flex flex-column">
                  <li>1000+ Edited Images</li>
                  <li>20-30 Sneak Picks</li>
                  <li>Engagment Session</li>
                  <li>5-6 min. Highlight Video</li>
                  <li>40-60 min. Full film</li>
                  <li>1 min. IG Reel</li>
                  <li>3-4 Video Cameras</li>
                  <li>3-4 Wireless Microphones</li>
                  <li>Aerial Drone Footege</li>
                  <li>Editing Style options</li>
                  <li>Multiple Locations</li>
                  <li>Online Photo Gallery</li>
                  <li>Digital Download</li>
                  <li>All print rights</li>
                  <li>Music License</li>
                </ul>
                <a href="#" class="btn btn-primary btn-lg d-flex align-items-center justify-content-center mt-auto w-100">
                  Get a quote
                  <svg class="svg-arrow-right" width="15" height="15" viewBox="0 0 24 24" fill="none">
                    <path d="M16.3153 16.6681C15.9247 17.0587 15.9247 17.6918 16.3153 18.0824C16.7058 18.4729 17.339 18.4729 17.7295 18.0824L22.3951 13.4168C23.1761 12.6357 23.1761 11.3694 22.3951 10.5883L17.7266 5.9199C17.3361 5.52938 16.703 5.52938 16.3124 5.91991C15.9219 6.31043 15.9219 6.9436 16.3124 7.33412L19.9785 11.0002L2 11.0002C1.44772 11.0002 1 11.4479 1 12.0002C1 12.5524 1.44772 13.0002 2 13.0002L19.9832 13.0002L16.3153 16.6681Z" fill="#0F0F0F"/>
                  </svg>
                </a>
              </div>
            </div>
          </div>
        </div>
        <div class="tab-pane fade" id="pricing-photo-tab" role="tabpanel" aria-labelledby="pricing-photo">
          
        </div>
        <div class="tab-pane fade" id="pricing-video-tab" role="tabpanel" aria-labelledby="pricing-video">
          
        </div>
      </div>
    </div>
  </div>
</section>