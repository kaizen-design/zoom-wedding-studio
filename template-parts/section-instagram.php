<?php if($instagram_feed = get_field('instagram_feed', 'option')): ?>
  <section class="instagram-section position-relative">
    <div class="carousel">
      <div class="group">
        <?php foreach($instagram_feed as $image): ?>
          <?php $thumbnail_url = wp_get_attachment_image_url( $image['ID'], 'thumbnail' ); ?>
    ...
    <?php if($instagram = get_field('instagram', 'option')): ?>
      <a href="<?= $instagram ?>" target="_blank" class="instagram-link d-flex align-items-center justify-content-center" data-aos="fade-up">
