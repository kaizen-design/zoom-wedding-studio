<section class="projects-section">
  <div class="container-xl d-flex flex-column">
    <h2 class="section-title d-flex flex-column align-items-center text-center mb-0">
      <span class="section-subtitle">View new our past work</span>
      Projects
    </h2>
    <div class="project-list row">
      <div class="col-lg-3">
        <a href="#" class="project-card card text-white">
          <img 
            src="<?= get_template_directory_uri() ?>/assets/tmp/project-1.webp" 
            class="card-img img-fluid"
            width="270"
            height="400"
            loading="lazy" 
            alt="Rachel & Nicholas" 
          />
          <div class="card-img-overlay d-flex flex-column align-items-center justify-content-end">
            <h3 class="card-title mb-0">Rachel & Nicholas</h3>
            <ul class="card-meta list-unstyled mb-0 d-flex justify-content-center">
              <li>8 hours</li>
              <li>Photo + Video</li>
            </ul>
          </div>
        </a>
      </div>
      <div class="col-lg-3">
        <a href="#" class="project-card card text-white">
          <img 
            src="<?= get_template_directory_uri() ?>/assets/tmp/project-2.webp" 
            class="card-img img-fluid"
            width="270"
            height="400"
            loading="lazy" 
            alt="Stephanie & Eric" 
          />
          <div class="card-img-overlay d-flex flex-column align-items-center justify-content-end">
            <h3 class="card-title mb-0">Stephanie & Eric</h3>
            <ul class="card-meta list-unstyled mb-0 d-flex justify-content-center">
              <li>10 hours</li>
              <li>Photo + Video</li>
            </ul>
          </div>
        </a>
      </div>
      <div class="col-lg-3">
        <a href="#" class="project-card card text-white">
          <img 
            src="<?= get_template_directory_uri() ?>/assets/tmp/project-3.webp" 
            class="card-img img-fluid"
            width="270"
            height="400"
            loading="lazy" 
            alt="Yeilem & Ivan" 
          />
          <div class="card-img-overlay d-flex flex-column align-items-center justify-content-end">
            <h3 class="card-title mb-0">Yeilem & Ivan</h3>
            <ul class="card-meta list-unstyled mb-0 d-flex justify-content-center">
              <li>4 hours</li>
              <li>Photo + Video</li>
            </ul>
          </div>
        </a>
      </div>
      <div class="col-lg-3">
        <a href="#" class="project-card card text-white">
          <img 
            src="<?= get_template_directory_uri() ?>/assets/tmp/project-4.webp" 
            class="card-img img-fluid"
            width="270"
            height="400"
            loading="lazy" 
            alt="Yola & Mark" 
          />
          <div class="card-img-overlay d-flex flex-column align-items-center justify-content-end">
            <h3 class="card-title mb-0">Yola & Mark</h3>
            <ul class="card-meta list-unstyled mb-0 d-flex justify-content-center">
              <li>8 hours</li>
              <li>Photo</li>
            </ul>
          </div>
        </a>
      </div>
    </div>
    <a href="#" class="btn btn-primary btn-lg d-flex align-items-center align-self-center">
      Show more
      <svg class="svg-arrow-right" width="15" height="15" viewBox="0 0 24 24" fill="none">
        <path d="M16.3153 16.6681C15.9247 17.0587 15.9247 17.6918 16.3153 18.0824C16.7058 18.4729 17.339 18.4729 17.7295 18.0824L22.3951 13.4168C23.1761 12.6357 23.1761 11.3694 22.3951 10.5883L17.7266 5.9199C17.3361 5.52938 16.703 5.52938 16.3124 5.91991C15.9219 6.31043 15.9219 6.9436 16.3124 7.33412L19.9785 11.0002L2 11.0002C1.44772 11.0002 1 11.4479 1 12.0002C1 12.5524 1.44772 13.0002 2 13.0002L19.9832 13.0002L16.3153 16.6681Z" fill="#0F0F0F"/>
      </svg>
    </a>
  </div>
</section>